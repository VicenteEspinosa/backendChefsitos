...
class TestViews(TransactionTestCase):
    ...
    def test_measurement_DELETE_by_id_when_item_exists(self):
        ...
        client = Client()
        client.force_login(User.objects.get_or_create(username='admin_test', is_superuser=True)[0])
    ...
    def test_measurement_DELETE_not_admin(self):
        initial_total = Measurement.objects.count()
        client = Client()
        client.force_login(User.objects.get_or_create(username='normal_user_test')[0])
        url = reverse('measurements', args=[1])
        response = client.delete(url)
